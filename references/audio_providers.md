...
- Use one consistent target-market voice within each final video; voices and delivery may vary across videos subject to the user's constraints, including all-female casting when requested. Query the provider's current voice/model catalog; do not assume the `gem-2.5-tts` voice-list endpoint or a hard-coded voice is valid for GEM-3.1-TTS.
- For N selected montage videos, author N content-matched complete scripts and batch-submit N independent TTS jobs with bounded concurrency. Keep a distinct durable journal and receipt per job, plus a manifest binding each variant to its script, voice, task, returned audio and timing. Resume submitted tasks without duplicating charges; never substitute one shared narration for the batch or split one long recording into N tracks.
...
- Derive cultural/musical context from the frozen market rather than hard-coding Japan. For a Japanese camping run, one example is `soft Japanese outdoor camping ambience, warm acoustic guitar and gentle pads, no vocals, no dramatic drops, evolving arrangement, natural ending, seamless-feeling bed`; another market needs its own approved brief. Do not request a fixed 30-second duration when runtime is narration-derived.
...
## Doubao TTS 2.0 (market-compatible voice audition / fallback)

- Provider: updrama, model `doubao-tts-2.0`, same `POST /v1/media/generate` and status polling contract.
- Query the live catalog with `GET /v1/skills/voices?model=doubao-tts-2.0`; choose a **market-profile-compatible, conversational commerce** preset for the frozen market. Do not hard-code the sample Chinese `zh_female_vv_uranus_bigtts` voice for a Japanese run.
...
- Use Doubao as a candidate voice, not an automatic replacement. Generate a short audition in GEM and Doubao, score natural prosody, target-language pronunciation, breath/pauses, warmth, and AI coloration, then select one voice for the complete narration. Re-derive runtime and repeat all sentence-tail/ASR QA after switching.
...
The only configured provider adapters in this repository are `gem`, `doubao`, and `suno`. Kinocut is a local renderer/mixer, not a music-generation model. No local MusicGen, Stable Audio, ACE-Step or other AI music checkpoint is installed; the `audiocraft` skill directory is documentation only. A future local-model installation must be a separate, explicitly approved task with its own dependency, checkpoint, hardware and license checks.
