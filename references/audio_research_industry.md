...
对于产品 UGC 投放，最终配乐不要烘进视频生成阶段。让 `video-use` 只做画面理解、卖点匹配和 EDL；随后单独生成一条完整日语旁白和一条无 vocals 的音乐轨，再由 Kinocut/FFmpeg 做 sidechain ducking、响度归一化和 QA。这样才能在不重剪画面的情况下替换音色、配乐、音量和版本风格。
