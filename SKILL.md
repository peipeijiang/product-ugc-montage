...
Write one complete, natural Japanese narration for the whole ad in the frozen market profile. Do not generate per-shot fragments and do not let shot selection rewrite the script after TTS. The default narration provider is **GEM-3.1-TTS** via the maintained updrama adapter; query the current catalog and choose a voice for natural prosody, human-like pauses, and low announcer/robotic coloration (for example, a relaxed preset for lifestyle UGC). Use one provider-supported voice for the entire run. Retain the task receipt, returned audio, script, and word-level timing. If the voice changes, re-derive runtime and re-run the full audio/visual QA; never splice two voices into one master.
